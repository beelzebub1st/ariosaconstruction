  ...
  return (
    <AdminShell title="Site Settings">
      <p className="mb-6 max-w-2xl text-sm text-muted">
        Update phone, email, address, map, hero copy, and about text. Requires a connected
        database (DATABASE_URL) for changes to appear on the live site.
      </p>
      <form
        action={async (fd) => {
          "use server";
      ...
      >
        ...
        <Field name="serviceArea" label="Service area label" defaultValue={settings.serviceArea} />
        ...
        <Field
          ...
          label="Experience label (shown on site)"
          defaultValue={settings.yearsExperience}
        />
        <Field
          name="trustBadges"
          label="Trust badges (pipe-separated)"
          defaultValue={settings.trustBadges}
        />
        ...
        <label className="block text-sm">
          <span className="mb-1.5 block font-medium text-navy">Hero support text</span>
          <textarea
            name="heroSupport"
            rows={2}
            defaultValue={settings.heroSupport}
        ...
        <label className="block text-sm">
          ...
          <textarea
            name="aboutLong"
            rows={5}
            defaultValue={settings.aboutLong}
            className="w-full rounded-md border border-navy/15 px-3 py-2.5"
          />
        </label>
        <Field
        ...
        <Field
          ...
          defaultValue={settings.googleUrl || ""}
        />
        <button
          type="submit"
